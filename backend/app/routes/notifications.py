...
async def send_price_alert_notification(request: PriceAlertRequest):
    ...
    if "email" in request.channels:
        try:
            ...
            # Calculate savings
            savings = (request.old_price - request.current_price) if request.old_price and request.old_price > request.current_price else 0
            ...
            email_service.send_alert_email(
                ...
                alert_type="price_drop" if request.condition == "below" else "price_change",
...
async def test_notification_services():
    ...
    try:
        ...
        results["websocket_service"]["active_connections"] = len(websocket_manager.active_connections)
    except Exception as e:
