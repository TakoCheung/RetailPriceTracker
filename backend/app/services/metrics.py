...
class ErrorMetricsService:
    ...
    def __init__(self):
        ...
        self.error_counts = defaultdict(lambda: defaultdict(int))  # error_type -> time_bucket -> count
        self.error_history = defaultdict(lambda: deque(maxlen=1000))  # error_type -> list of timestamps
        self.error_contexts = defaultdict(list)  # error_type -> list of context data
        self.thresholds = {}  # error_type -> threshold config
    
    def track_error(self, error_type: str, component: str, context: Optional[Dict[str, Any]] = None):
    ...
    def track_error_with_context(self, error_type: str, component: str, context: Dict[str, Any]):
        """Track error with additional context for correlation analysis."""
        self.track_error(error_type, component, context)
    
    def get_error_rates(self, time_window_minutes: int = 60) -> Dict[str, Dict[str, Any]]:
    ...
    def set_error_threshold(self, error_type: str, count: int, time_window_minutes: int):
            ...
            if error_type in rates:
                ...
                if current_count >= threshold["count"]:
                    alert = {
                        ...
                        "severity": "critical" if current_count >= threshold["count"] * 2 else "warning",
    ...
    def _get_top_errors(self, error_rates: Dict[str, Dict[str, Any]], limit: int = 5) -> List[Dict[str, Any]]:
