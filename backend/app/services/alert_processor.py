...
class AlertProcessingService:
    ...
    async def _trigger_alert(
        ...
        for channel in alert.notification_channels:
            await self._send_notification(
                channel, user, product, alert_data, current_price, old_price
            )
        ...
        try:
            if channel == "email":
                await self._send_email_notification(
                    user, product, current_price, old_price, alert_data
                )
            elif channel == "sms":
    ...
    async def _send_email_notification(
        ...
        alert_type = (
            "price_drop" if old_price and current_price < old_price else "price_alert"
        )
    ...
    async def process_new_price_record(
        self, db_session: AsyncSession, price_record: PriceRecord
    ) -> Dict[str, Any]:
        ...
        return await self.process_price_change(
            db_session, price_record.product_id, price_record.price, old_price
        )
