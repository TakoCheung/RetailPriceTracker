...
from app.models import Product, PriceRecord, ProductStatus, Provider
...
class ProductService:
    ...
    def __init__(self):
        self.supported_categories = [
            ...
            "Health & Beauty", "Automotive", "Toys", "Food", "Other"
        ]
    ...
    async def create_product(
        ...
        if category and category not in self.supported_categories:
            raise ValueError(f"Category must be one of: {', '.join(self.supported_categories)}")
        
        # Check for duplicate SKU if provided
        if sku:
            existing = await db_session.execute(
                select(Product).where(and_(Product.sku == sku, Product.deleted_at.is_(None)))
            )
    ...
    async def search_products(
        ...
        results_query = base_query.order_by(desc(Product.updated_at)).limit(limit).offset(offset)
    ...
    async def update_product(
        ...
        **updates
        ...
        if "category" in updates and updates["category"] not in self.supported_categories:
            raise ValueError(f"Category must be one of: {', '.join(self.supported_categories)}")
    ...
    async def get_product_price_history(
        ...
        query = (
            ...
            .where(PriceRecord.recorded_at >= datetime.now(timezone.utc).date() - timedelta(days=days))
        )
    ...
    async def _notify_price_change(
        ...
        if abs(change_percent) > 10:  # More than 10% change
            print(f"Significant price change detected: {product.name} - {change_percent:.1f}%")
